...
data_x_values = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_x_values.csv', delimiter=',')  # x values are common for both data and model
data_y_values = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_y_values.csv', delimiter=',')  # csv derived from histogram_get_bin_content_error.C
data_y_values_var = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_y_values_var.csv', delimiter=',')  # csv derived from histogram_get_bin_content_error.C
data_x_values_fullrange = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_x_values_fullrange.csv', delimiter=',')  # for visualization only
data_y_values_fullrange = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_y_values_fullrange.csv', delimiter=',')  # for visualization only
model_y_values = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_model_y_values.csv', delimiter=',')  # csv derived from Comparison_DSL2.C
...
model_y_values_fullrange = np.loadtxt(dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_model_y_values_fullrange.csv', delimiter=',')  # for visualization only

print("Path: ", dir_path + '\DSL_' + peak + fakeTau + '\DSL_' + peak + '_data_x_values.csv')
...
p1 = ax_prior.errorbar(data_x_values_fullrange, data_y_values_fullrange, yerr=[lower_errors, upper_errors], fmt='s', color='black', markersize=1, label='Data', ecolor='black', zorder=2)  # zorder 2 appears on top of the zorder = 1.
...
p2 = ax_prior.fill_between(data_x_values_fullrange, smoothed_model_y_value_low, smoothed_model_y_value_high,
...
axs_emu2[0].scatter(model_y_values_test.T, pred_m, alpha=0.5)
if peak == '31S1248':
    bin_count_max = 90
if peak == '31S3076':
    bin_count_max = 12
...
axs_emu2[1].scatter(model_y_values_train.T, pred_m_tr, alpha=0.5)
...
axs_emu3[0].hist((pred_m - model_y_values_test.T).flatten(), bins=100)
...
axs_emu3[2].hist(((pred_m - model_y_values_test.T)/model_y_values_test.T).flatten(), bins=100)  # relative error
...
if peak == '31S4156':
    calibrator_1 = calibrator(emu=emulator_1,
                                               ...
                                               # method='mlbayeswoodbury',
                                               yvar=obsvar,
                                               args={'theta0': np.array([[10.0, 4155.84, 1.0, 1.0]]),  # initial guess ['Tau', 'Eg', 'Bkg', 'SP']
                                                          'sampler': 'metropolis_hastings',
                                                          # 'sampler': 'LMC',
                                                          # 'sampler': 'PTLMC',
                                                          'numsamp': number_of_mcmc_samples,
...
def plot_pred_interval(calib):
    ...
    # Upper Poisson errors
    upper_errors = poisson.ppf(0.84, data_y_values_fullrange) - data_y_values_fullrange
    p1 = ax_post_predict.errorbar(data_x_values_fullrange, data_y_values_fullrange, yerr=[lower_errors, upper_errors], fmt='s', color='black', markersize=1, label='Data', ecolor='black', zorder=2)  # zorder 2 appears on top of the zorder = 1.
    ...
    p2 = ax_post_predict.plot(data_x_values, posterior_y_median, color='blue', alpha=1.0, zorder=1)
    p3 = ax_post_predict.fill_between(data_x_values, posterior_y_lower, posterior_y_upper, color='blue', alpha=0.3, linewidth=0, zorder=1)
    ...
    ax_post_predict.legend([p1[0], p2[0], p3], ['Data', 'Prediction Mean', '95% Credible Interval'], fontsize=24)
    xmin = min(data_x_values_fullrange) + 70
    xmax = max(data_x_values_fullrange) - 89.5
...
g.fig.suptitle("Posterior", fontfamily="Times New Roman", fontsize=18)
for ax in g.fig.axes:
...
g.fig.legend(labels=['Posterior', 'Prior'], fontsize=24)
