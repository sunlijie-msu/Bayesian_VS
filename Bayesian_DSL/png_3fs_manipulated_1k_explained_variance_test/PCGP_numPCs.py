...
def fit(fitinfo, x, theta, f, epsilon=0.5, **kwargs):
    '''
    ...
    fitinfo['x'] = x
    fitinfo['f'] = f
    fitinfo['epsilon'] = epsilon # Typically, we want to keep the principal components that capture the most variance. The epsilon parameter is used to set a threshold for filtering these singular values.

    # Extract num_pcs from args
    num_pcs = kwargs.get('num_pcs', None) # Extract num_pcs from args. Added by Lijie Sun
    ...
    # __PCs(fitinfo)
    __PCs(fitinfo, num_pcs=num_pcs) # Pass num_pcs to the __PCs function. Changed by Lijie Sun
    numpcs = fitinfo['pc'].shape[1]
    fitinfo['singular_values'] = fitinfo['S']  # Store the singular values in fitinfo for later visualization. Added by Lijie Sun
...
def __PCs(fitinfo, num_pcs=None): # Added num_pcs parameter. Changed by Lijie Sun
    "Apply PCA to reduce the dimension of f"
    # Extracting from input dictionary
    f = fitinfo['f']
    fs = fitinfo['fs']
    ...
    if num_pcs is not None: # Added by Lijie Sun
        # Select the specified number of principal components
        pct = U[:, :num_pcs]
        pcw = S[:num_pcs]
    else:
        # Use epsilon to determine the number of principal components
        Sp = S ** 2 - epsilon
        # S: Contains the singular values of the matrix fs.T (i.e., the square roots of the eigenvalues of the covariance matrix fs.T)
        pct = U[:, Sp > 0]
        pcw = np.sqrt(Sp[Sp > 0])

    pcstdvar = np.zeros((f.shape[0], pct.shape[1]))
    fitinfo['S'] = S  # Add singular values to fitinfo # Added by Lijie Sun
    fitinfo['pcw'] = pcw
    fitinfo['pcto'] = 1*pct
